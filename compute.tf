...
resource "aws_launch_template" "ecommerce_lt" {
  ...
  user_data = base64encode(<<-EOF
    #!/bin/bash
    dnf update -y
    dnf install -y httpd wget unzip php php-cli php-mysqlnd php-gd php-curl php-xml php-mbstring php-intl php-zip
    sed -i 's/display_errors = On/display_errors = Off/' /etc/php.ini
    sed -i 's/error_reporting = E_ALL/error_reporting = E_ALL \& ~E_DEPRECATED \& ~E_STRICT/' /etc/php.ini
